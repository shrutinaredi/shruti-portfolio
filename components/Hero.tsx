  ...
  return (
    <section id="hero" className="relative min-h-screen flex items-center pt-24 pb-16 overflow-hidden dot-grid noise">
      <div className="absolute inset-0 pointer-events-none overflow-hidden">
        <div className="absolute -top-32 -left-32 w-[600px] h-[600px] rounded-full bg-emerald/10 blur-[140px] animate-float" />
        <div className="absolute top-1/2 -right-40 w-[500px] h-[500px] rounded-full bg-emerald/8 blur-[120px] animate-float-delay" />
        <div className="absolute -bottom-20 left-1/3 w-[400px] h-[400px] rounded-full bg-emerald-dark/10 blur-[100px] animate-float-delay2" />
      </div>
        ...
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-8 items-center">
          <div>
            <motion.div
              ...
              className="inline-flex items-center gap-2.5 px-4 py-1.5 rounded-full glass border border-emerald/20 text-sm text-muted mb-8"
            >
            ...
            >
              <a href="#projects"
                className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-emerald hover:bg-emerald-dark text-white font-semibold text-sm transition-all duration-200 hover:shadow-lg hover:shadow-emerald/30 hover:-translate-y-0.5"
              >
              ...
              <a href="mailto:sagarwal235@wisc.edu"
                className="inline-flex items-center gap-2 px-6 py-3 rounded-xl border border-border text-muted hover:border-emerald/60 hover:text-emerald-light font-semibold text-sm transition-all duration-200 hover:-translate-y-0.5"
              >
                ...
                <div className="p-6 font-mono text-sm leading-[1.9] bg-bg2">
                  {CODE_LINES.map((line, i) => (
                    ...
                    >
                      ...
                      {line.t === "keyword" && <div><span className="text-emerald-light font-semibold">{line.text}</span><span className="text-text">{line.rest}</span></div>}
                      {line.t === "var"     && <div><span className="text-emerald-light">{line.text}</span><span className="text-muted">{line.rest}</span></div>}
                      {line.t === "prop"    && <div><span className="text-emerald-light">{line.text}</span><span className="text-muted">=</span><span className="text-amber-light">{line.val}</span></div>}
                      {line.t === "call"    && <div><span className="text-emerald">{line.text}</span><span className="text-muted">{line.suffix}</span></div>}
                    </motion.div>
                  ))}
                  <div className="mt-1 text-emerald flex items-center">
                    <span>&#8250;</span>
                    <span className="animate-blink ml-1 inline-block w-2 h-[18px] bg-emerald align-middle" />
                  </div>
                </div>
              </div>
              <motion.div animate={{ y: [0, -8, 0] }} transition={{ repeat: Infinity, duration: 3, ease: "easeInOut" }}
                className="absolute -bottom-5 -left-6 glass rounded-xl px-4 py-2.5 border border-emerald/30 text-xs font-mono text-emerald-light shadow-xl"
              >
                GPA: 4.0 &nbsp;&middot;&nbsp; 0 vulnerabilities
              </motion.div>
              <motion.div animate={{ y: [0, -6, 0] }} transition={{ repeat: Infinity, duration: 4, delay: 1, ease: "easeInOut" }}
                className="absolute -top-5 -right-4 glass rounded-xl px-4 py-2.5 border border-emerald/30 text-xs font-mono text-emerald-light shadow-xl"
              >
      ...
      >
        ...
        <motion.div animate={{ y: [0, 7, 0] }} transition={{ repeat: Infinity, duration: 1.5, ease: "easeInOut" }}
          className="w-px h-7 bg-gradient-to-b from-emerald/70 to-transparent"
        />
      </motion.div>
    </section>
  );
